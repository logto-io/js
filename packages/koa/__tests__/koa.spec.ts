...
describe('cookie strategy', () => {
    ...
    beforeAll(() => {
        app = new Koa();
        app.use(logto({ strategy: "cookie" }));
        app.use((ctx: Context) => {
            ctx.body = 'hello ' + ctx.user.id;
        });
    })
    test('find user', (done) => {
        request.agent(app.callback()).get('/').set('cookie', 'logto_token=test-user-token').expect('hello test-user', done);
    });
    test('empty token should get 401', (done) => {
        request.agent(app.callback()).get('/').expect(401, done);
    });
    test('invalid token should get 401', (done) => {
        request.agent(app.callback()).get('/').set('cookie', 'logto_token=test-user-token1').expect(401, done);
    });
});
